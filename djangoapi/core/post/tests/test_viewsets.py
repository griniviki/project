...
class TestPostViewSet:
    endpoint = '/api/post/'  # ✅ Correct endpoint
    ...
    def test_list(self, client, user, post):
        ...
        assert len(response.data) == 1  # ✅ No pagination, just check list length
    ...
    def test_update(self, client, user, post):
        ...
        response = client.put(f"{self.endpoint}{post.public_id}/", data, format='json')
        assert response.status_code == status.HTTP_200_OK
        assert response.data['body'] == data['body']
